...
def mtmc_clustering(tracks: List[List[Tracklet]],
    ...
    for i, cam_tracks in enumerate(tracks):
        for track in cam_tracks:
            ...
            if cams:
                track.global_start = track.frames[0] / \
                    cams.fps[i] / cams.scales[i] + cams.offset[i]
                track.global_end = track.frames[-1] / \
                    cams.fps[i] / cams.scales[i] + cams.offset[i]
    ...
    log.info(
        f"Precomputation finished, took {time.time() - log_start_time:.3f} s.")

    # initialize multicam tracklets
    mtracks = [MulticamTracklet(i, [all_tracks[i]], len(tracks))
               for i in range(n)]
    ...
    while len(merge_queue) > 0:
        ...
        for i_other in remaining_tracks:
            if i_other == i1:
                continue
            if not any_compatible(mtracks[i1], mtracks[i_other]):
                continue
            s = multicam_track_similarity(
                mtracks[i1], mtracks[i_other], linkage, sim)
            if s >= min_sim:
